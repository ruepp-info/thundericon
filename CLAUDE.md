...
Rebuilding on the idle flush leaves those rows badge-less for one or more painted frames. So `onMutations` hands every row the batch touched to `renderNow`, which `decorate`s the **badge-less** ones synchronously: observer callbacks are microtasks, so the badge lands in the same frame as the row content and the empty frame never reaches the screen. Rows that already have a badge stay on the idle path (their re-render is at worst a signature check, never a hole). `senderKnown()` skips rows the tree hasn't filled *and* that `gDBView` can't name, so we never paint a `?` and correct it later. `renderNow` dequeues a row **before** `decorate`s it — a host that answers `resolveBimi`/`resolveGravatar` synchronously re-enqueues it from inside `decorate` to swap the image in. `dropSelfRecords()` (an `observer.takeRecords()` after our own DOM writes, in both `renderNow` and `flush`) keeps our badge insertions from waking another flush.
