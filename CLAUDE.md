...
The `npm test` script is `node --test test/*.test.js` — an **intentionally unquoted** glob so the POSIX shell expands it to the four real test files (Node's own glob handling for test specifiers varies by version). Do **not** change it to `node --test` (auto-discovery) or `node --test test/`: both match `options/bimi-test.js` — the options UI script, whose name fits Node's `*-test.js` discovery pattern — which then throws on load and breaks the run.
