...
### Versioning & releases
Version is **tag-driven**, so there is no need to hand-bump for a release. The committed `version` in `manifest.json` + `package.json` is a deliberate dummy (`0.0.0`) — don't rely on it matching releases. `tools/package.py` reads the version only from `manifest.json` and names the artifact `thundericon-<version>.xpi`; `options.js` reads the running version from the manifest at runtime. On a published GitHub Release, `.github/workflows/release.yml` derives the version from the tag (read from `github.event.release.tag_name`; `v1.2.3` or `1.2.3`), injects it into both files at build time (**not** committed back), runs `npm ci && npm test`, packages, and uploads the `.xpi`. `.github/workflows/ci.yml` runs `npm ci && npm test` + `package.py` validation on every push to `main`/`master` and on PRs. Both workflows run on Node 24 (`actions/*@v5`). The `release` workflow runs from the **default branch's** copy, so its file must be on the default branch for releases to build. Full procedure: `RELEASE.md`.

**`strict_max_version` is deliberately uncapped (`999.*`) and is NOT rewritten by the build.** Gecko applies strict compatibility to *every* WebExtension (`XPIInstall.sys.mjs` sets `addon.strictCompatibility = true` unconditionally; an absent cap defaults to `"*"`), so a cap below the running Thunderbird disables the add-on outright — no warning, just off. The release workflow used to pin the cap to the Thunderbird release of the build day, which meant the *next* monthly Thunderbird silently disabled every shipped build; that is exactly what happened on TB 153 against a committed `152.*`. The workflow step now only emits a `::warning::` if the committed cap has fallen below the current release. Do **not** reintroduce auto-pinning. The trade this accepts: an Experiment reaching into non-stable internals can break on a future Thunderbird instead of refusing to load — that fails soft (selectors stop matching, no crash), so smoke-testing on new majors is the safety net the cap used to be.
...
**The one exception to idle batching** is `renderNow`, which exists to kill folder-switch flicker. A row that should carry a badge but doesn't is a *visible hole* — in Cards layout the `rowTint`/`fill` background hangs off `td:has(> .ti-avatar--card)`, so a missing badge un-paints the whole row. Thunderbird destroys badges far more often than it looks:

- `TreeViewTableBody.reset()` does `table.body.replaceChildren()`, discarding every row element and every badge in it. A folder switch runs it **at least twice** — once from `set view`, then again when the db view calls `_jsTree.invalidate()` (which is `threadTree.reset()`, not an in-place update) as the folder finishes loading.
- In Table layout, `ThreadRow.fillRow()` assigns `cell.textContent`, deleting the badge in that cell.
- `TreeViewTableRow`'s `set index` defers `fillRow()` to a `requestAnimationFrame`, so a row can reach the observer before it has any content. Both row classes clone their template in `connectedCallback`, so the `<td>` cells do exist by then.

Thunderbird also refills rows **in place** (`invalidateRow` on a mark-as-read, new mail, a re-sort) and un-bolds the card's text in the same animation frame; a Cards badge survives that, but its `--unread`/`--read` marker — and with it the whole `rowTint` background — has to follow in the same frame.

Deferring any of this to the idle flush costs one or more painted frames. So `onMutations` hands **every** row the batch touched to `renderNow`, which `decorate`s them synchronously: observer callbacks are microtasks, so the work lands in the same frame as the row content the tree just wrote. `decorate` is signature-guarded, so an unchanged row costs one header read plus a `describe`; and this does not move the scroll budget, because scrolling makes the tree build *new* row elements rather than touch drawn ones. Notes:

- `resolveSender(row)` resolves `{ hdr, author }` once and is passed into `decorate` (the idle flush lets `decorate` resolve it itself). A row with neither is one the tree hasn't filled *and* `gDBView` can't name — skipped, so we never paint a `?` and correct it later.
- `renderNow` dequeues a row **before** `decorate`s it: a host that answers `resolveBimi`/`resolveGravatar` synchronously re-enqueues it from inside `decorate` to swap the image in, and that must survive.
- `dropSelfRecords()` (an `observer.takeRecords()` after our own DOM writes, in both `renderNow` and `flush`) keeps our badge insertions from waking another flush.
- `SYNC_RENDER_MAX` caps a batch; the overflow falls back to the idle flush.
- Table rows are decorated twice per creation — once on insert, once after `fillRow()` deletes the badge a frame later. Both land pre-paint, so this is wasted work, not a defect. Detecting "the tree has filled this row" would need a TB internal (`dataset.properties`, `row.id`) whose disappearance would mean *no badge at all* rather than one extra render; not worth the trade.

Still asynchronous by nature: a BIMI logo or Gravatar photo that is not yet resolved renders as initials first and swaps in when the host answers.

**Unread subject color** (both layouts) is the one unread feature that is *not* built on the avatar. It is orthogonal to `unreadStyle` (combines with any cue, or none) and gates on its own root attribute `:root[data-ti-unread-subject]` + `--ti-unread-subject-color`, both written by `applyConfig`; `decorate` is untouched. The CSS can't key off our badge — in Table view the badge sits in the correspondent cell, not the subject cell — so it keys off **Thunderbird's own** per-row `tr[data-properties~="unread"]`, the same hook `threadPane.css` uses for the default bold. Consequences: it also colors scraped rows (no `gDBView` → no marker class), and because no badge class is involved it must be gated on the master `enabled` switch explicitly (`removeAllBadges` would otherwise leave it painting). The subject elements are internal DOM — Cards `.thread-card-subject-container > .subject`, Table `td.subjectcol-column .subject-line > span` (target the span; the sibling `<img>` is the message-state icon and would inherit the color) — verify on major TB updates. Selected rows are excluded (TB paints its own contrast color on the selection background), as are tagged rows in Table view (TB deliberately lets the tag color win there). `avatars.css` is an AUTHOR sheet loaded after `threadPane.css` and these rules target the subject element directly while TB sets `color` on the `<tr>`, so they win without `!important`.

**Unread emphasis** (Cards layout only): `decorate` reads `hdr.isRead` synchronously (no host bridge), tags the card badge `ti-avatar--unread`/`ti-avatar--read` (never both; unknown/scraped rows get neither), and folds the read state into the render signature so a mark-as-read flip repaints. `applyConfig` writes the accent color (`--ti-unread-accent`), the accent-bar thickness (`--ti-unread-bar-width`, from `unreadBarWidth` → `narrow`/`medium`/`wide`), the `glyph`-style character + its font/size/weight (`--ti-unread-glyph*`; the char is emitted as a CSS-`content` quoted string via `glyphContent()`), the `fill`-style background (`--ti-unread-fill` + its contrast-picked `--ti-unread-fill-fg`, with `data-ti-fill-mode` = `fixed`/`iconColor`), and which cues to show as space-separated tokens on `:root[data-ti-unread-style]` (e.g. `"bar fade"`); the CSS rules gate on those tokens and are scoped to `tr[is="thread-card"]`, so Table view is untouched. The `bar`/`dot`/`glyph` cues all share the cell's single `td::before` — the tokens are mutually exclusive presets, so they never collide. The `fill` style paints a colour behind the icon across the whole avatar column — a full-height `td::before` from the leading edge to the card text (`size*1.7 + 16px` wide) — leaving the icon itself untouched (real per-sender colour/logo/photo, normal shape); the absolutely-positioned badge naturally paints over the ::before. Fixed mode uses the fixed colour; icon-colour mode uses the per-sender colour (`--ti-row-color`) for initials rows (icon sits on a same-colour block) and the fixed colour for image badges (BIMI/Gravatar), which have no single colour. The `rowTint` style reuses the same mode/colour but washes the whole card cell with a precomputed `#rrggbbaa` colour at the `unreadRowStrength` alpha (100 = exact/opaque, lower = a lighter wash that lets TB's selection/hover/alternating-row backgrounds read through). The renderer computes the wash via `washHex()` — `--ti-unread-fill-wash` (root, fixed mode) and `--ti-row-wash` (per cell, sender colour) — **not** CSS `color-mix()`, whose `var()` percentage does not resolve in this Gecko. Icon-colour mode uses `--ti-row-wash`/`--ti-row-color` (published per cell); image rows fall back to the fixed wash. Live update relies on TB rewriting the card's bold text (a `characterData`/`childList` mutation) on mark-read; if a future TB flips read state via a class-only change, add a scoped `attributeFilter` to the observer.
